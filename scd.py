...
"""
Problem:
Conference room scheduling. Find the nearest open conference room for a team in which a team can hold 
its meeting. 

Given n team members with the floor on which they work and the time they want to meet, and a list 
of conference rooms identified by their floor and room number as a decimal number, maximum number of 
people it fits and pairs of times they are open - find the best place for the team to have their 
meeting. If there is more than one room available that fits the team at the chosen time then the 
best place is on the floor the closest to where the team works.
E.g.
rooms.txt
...
Please explain: how you solved the problem and how it would behave based on the different parameters 
(number of team members, longer meeting times, many rooms with random booking times). How would you 
test the program to ensure it always produced the correct results?

For extra credit, can you improve the solution to split the meeting across more than one room if 
say only one room is available for a fraction of the meeting and another room is free later to hold the 
remainder of the meeting during the set time. If you want to make this more powerful - assume that the 
number of room splits can happen in proportion to the length of the meeting so that say if a meeting 
is 8 hrs long then the algorithm could schedule it across say up to 4 rooms if a single room was not available for the whole time.
You may code the response in any programming language you like, however our primary DevOps programming 
languages are:
•	Bash
•	Perl
•	Python
...
class Room:
    ...
    def get_room_info(self):
        ...
        print('\t(', self.floor, '.', self.room_no, ') capacity =', self.capacity)
    ...
    def add_booking_time(self, str_start_time, str_end_time):
        ...
        if(len(end_tm_lst)<=1):
            print('ERROR: add_booking_time ->', dbg_lno())
            return None
        n_dt_start = dtm_start.replace(microsecond=0,second=0, hour=int(start_tm_lst[0]), minute=int(start_tm_lst[1]))
        n_dt_end = dtm_end.replace(microsecond=0,second=0, hour=int(end_tm_lst[0]), minute=int(end_tm_lst[1]))
    ...
    def reserve_time_slot(self, room_idx, dt_key, val=True):
...
class RoomScheduler:
    ...
    def get_avail_multi_room(self, num_team_mtg, floor_team_mtg, n_schd_dt_start, n_schd_dt_end):
        global g_rooms_list
        room_is_found = False
        one_pass = False
        done = False
        ...
        while (i < len(g_rooms_list) and done == False):
            if (g_rooms_list[i].get_room_capacity() >= int(num_team_mtg)):
                ...
                for index, key in enumerate(g_rooms_list[i].bk_time_dict):
                    val_list = g_rooms_list[i].bk_time_dict[key]
                    # check if the meeting can terminate without another room
                    if ((one_pass == False) and (key <= n_schd_dt_start) and 
                        (val_list[0] >= n_schd_dt_end) and (val_list[1] != True)):
                        room_is_found = True
                    ...
                    # intermediate room checking
                        ...
                        and (val_list[1] != True)):
                        # check the next slot
                        if (val_list[0] < n_schd_dt_end and val_list[0] < n_schd_dt_start):
                ...
                if (done == False and room_is_found == True):
                    ...
                    if (n_schd_dt_end > cur_room_end_time and one_pass == True):
    ...
    def schdl_multi_room(self, inp):
        ...
        n_schd_dt_start = schd_start.replace(microsecond=0,second=0,hour=int(schd_start_tm_lst[0]), minute=int(schd_start_tm_lst[1]))
        n_schd_dt_end = schd_end.replace(microsecond=0,second=0, hour=int(schd_end_tm_lst[0]), minute=int(schd_end_tm_lst[1]))
        resv_rms = self.get_avail_multi_room(num_team_mtg, floor_team_mtg, n_schd_dt_start, n_schd_dt_end)      
...
def read_input_file(file_name):
    ...
    for line in file.readlines():
        ...
        if ('.' not in room_data[0]):
            continue
        if not (room_data) or ((len(room_data) < 4) or (len(room_data) % 2 != 0)):
            continue
        # input is parsed    
        print ('[',idx,']')
...
if __name__ == '__main__':
    ...
    dtm_start = datetime.datetime.now()
    n_dt_start = dtm_start.replace(microsecond=0,second=0, hour=10, minute=30)   
    ...
    print ('\nUnreserve the time slots:')
    dtm_start = datetime.datetime.now()
    n_dt_start = dtm_start.replace(microsecond=0,second=0, hour=15, minute=15)   
    ...
    if (resv_rms == None):
        ...
        print ('\nUnreserve the time slots:')
        dtm_start = datetime.datetime.now()
        n_dt_start = dtm_start.replace(microsecond=0,second=0, hour=16, minute=30)   
        resv_rms[0].reserve_time_slot(9, n_dt_start, False)
        n_dt_start = dtm_start.replace(microsecond=0,second=0, hour=13, minute=30)   
        resv_rms[0].reserve_time_slot(7, n_dt_start, False)
        n_dt_start = dtm_start.replace(microsecond=0,second=0, hour=17, minute=30)  
    ...
    if (resv_rms == None):
        ...
        print ('\nUnreserve the time slots:')
        dtm_start = datetime.datetime.now()
        n_dt_start = dtm_start.replace(microsecond=0,second=0, hour=9, minute=0)   
        resv_rms[0].reserve_time_slot(2, n_dt_start, False)
        n_dt_start = dtm_start.replace(microsecond=0,second=0, hour=9, minute=15)   
        resv_rms[0].reserve_time_slot(12, n_dt_start, False)
        n_dt_start = dtm_start.replace(microsecond=0,second=0, hour=12, minute=0)  
        resv_rms[0].reserve_time_slot(1, n_dt_start, False)
        n_dt_start = dtm_start.replace(microsecond=0,second=0, hour=15, minute=15)  
